 ...
 * → Warn; 204/205 with none, or only `identity` (not a real coding — that is R1's concern, not this
 * clause's) → Pass; 304/200/206 → Skip(NotApplicable) — the clause only speaks to the bodiless
 * statuses.
 */
export const noContentEncodingOnBodilessResponse = defineResponseRule({
  ...
  judge(exchanges) {
    ...
    return isCompressed(exchange.head) ? { verdict: Verdict.Warn } : { verdict: Verdict.Pass };
  },
});
