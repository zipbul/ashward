...
test('warns when the hostile malformed-percent vector 5xxs after a stable control', async () => {
  const out = await malformedPercentNoHardFail.run({
...
test('is tagged with CWE-20', () => {
  expect(malformedPercentNoHardFail.tags?.cwe).toEqual(['CWE-20']);
});
