...
import type { HttpRuleContext, HttpTarget, ProbeFn } from './context';
...
/** Run HTTP-domain rules against a target over the default TCP transport, into a Report. */
export async function runHttp(target: HttpTarget, rules: readonly RuleDef<HttpRuleContext>[]): Promise<Report> {
  return runRules<HttpRuleContext>({ probe: bindHttpProbe(target), target }, rules);
}
