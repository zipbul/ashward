...
test('classifies a 2xx response as accepted', () => {
  expect(classifyFramingOutcome({ statusLine: status(200), termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Accepted,
  );
});

test('classifies status 399 (adjacent to the 400 boundary) as accepted', () => {
  expect(classifyFramingOutcome({ statusLine: status(399), termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Accepted,
  );
});

test('classifies status 400 (the rejection boundary) as rejected', () => {
  expect(classifyFramingOutcome({ statusLine: status(400), termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Rejected,
  );
});

test('classifies status 599 (upper server-error boundary) as rejected', () => {
  expect(classifyFramingOutcome({ statusLine: status(599), termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Rejected,
  );
});
...
test('classifies a clean close with no response as rejected', () => {
  expect(classifyFramingOutcome({ statusLine: null, termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Rejected,
  );
});

test('classifies an aborted connection with no response as rejected', () => {
  expect(classifyFramingOutcome({ statusLine: null, termination: TerminationCause.Rst })).toBe(
    FramingOutcome.Rejected,
  );
});

test('classifies a timeout with no response as inconclusive', () => {
  expect(classifyFramingOutcome({ statusLine: null, termination: TerminationCause.Timeout })).toBe(
    FramingOutcome.Inconclusive,
  );
});
...
test('classifies a 3xx redirect as accepted (the frame was processed)', () => {
  expect(classifyFramingOutcome({ statusLine: status(301), termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Accepted,
  );
});

test('classifies a mid-range 5xx as rejected', () => {
  expect(classifyFramingOutcome({ statusLine: status(500), termination: TerminationCause.Fin })).toBe(
    FramingOutcome.Rejected,
  );
});
